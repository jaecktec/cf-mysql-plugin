...
var _ = Describe("HttpWrapper", func() {
	...
	Describe("Get", func() {
		Context("When SSL is disabled", func() {
			It("Configures an HTTP client without cert validation", func() {
				mockFactory := new(cfpsqlfakes.FakeHttpClientFactory)
				httpWrapper := cfpsql.NewHttpWrapper(mockFactory, new(netfakes.FakeRequestDumperInterface))
	...
	Describe("Post", func() {
		It("Sends a body and sets the content type to application/json", func() {
			...
			httpWrapper := cfpsql.NewHttpWrapper(cfpsql.NewHttpClientFactory(), dumper)
...
func MakeHttp() cfpsql.HttpWrapper {
	return cfpsql.NewHttpWrapper(cfpsql.NewHttpClientFactory(), new(netfakes.FakeRequestDumperInterface))
}
